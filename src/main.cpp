...
{
#if defined(_DEBUG)
	// Always enable the debug layer before doing anything DX12 related so all possible errors generated
	// while created DX12 objects are caught by the debug layer
...
ComPtr<ID3D12Device2> CreateDevice(ComPtr<IDXGIAdapter4> adapter)
{
	...
	if (SUCCEEDED(d3d12Device2.As(&pInfoQueue)))
	{
		...
		D3D12_MESSAGE_ID DenyIds[] = {
			D3D12_MESSAGE_ID_CLEARRENDERTARGETVIEW_MISMATCHINGCLEARVALUE,	// This warning occurs when not using the optimized clear color to clear a render target
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (g_IsInitialized)
	{
		...
		break;
		// The default window procedure will play a system notification sound when pressing the Alt+Enter
		// keyboard combination if this message is not handled.
		case WM_SYSCHAR:
			break;
		case WM_SIZE:
		{
